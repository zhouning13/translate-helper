...
@Service
public class ArmorFileService implements IJsonFileService {
	...
	public List<FileMeta> getFileMeta(String fileName, JsonNode tn) {
		...
		for (JsonNode n : tn) {
			if (n != null && !n.isNull()) {
				fm.add("$[" + i + "].name", new SectionInfo(n.get("name").textValue(), "装备" + i + "名称"));
				fm.add("$[" + i + "].description", new SectionInfo(n.get("description").textValue(), "装备" + i + "描述"));
			}
			i++;
		}
