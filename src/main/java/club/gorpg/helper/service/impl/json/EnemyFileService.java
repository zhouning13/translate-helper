...
@Service
public class EnemyFileService implements IJsonFileService {
	private static final String FILE_NAME = "data/Enemies.json";
	...
	public List<FileMeta> getFileMeta(String fileName, JsonNode tn) {
		FileMeta fm = new FileMeta(FILE_NAME, FileIcon.enemy, "敌人");
		int i = 0;
		for (JsonNode n : tn) {
			if (n != null && !n.isNull()) {
				fm.add("$[" + i + "].name", new SectionInfo(n.get("name").textValue(), "敌人" + i));
			}
			i++;
		}
